...
mod tests {
    use super::*;
    use crate::config::DEFAULT_SECURITY_LEVEL;
    ...
    fn test_dilithium_keygen() {
        ...
        assert_eq!(keypair.public_key.t.len(), dilithium.config.k);
        assert_eq!(keypair.private_key.s1.len(), dilithium.config.l);
        assert_eq!(keypair.private_key.s2.len(), dilithium.config.k);
    }
    ...
    fn test_dilithium_sign_verify() {
        ...
        let signature =
            dilithium.sign(message, &keypair.private_key, None).unwrap();
        let is_valid =
    ...
    fn test_invalid_signature() {
        ...
        let signature =
            dilithium.sign(message, &keypair.private_key, None).unwrap();
        let is_valid =
            dilithium.verify(wrong_message, &signature, &keypair.public_key);
