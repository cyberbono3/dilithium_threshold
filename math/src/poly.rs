...
// Simple modular reduction that handles negative numbers correctly
#[inline(always)]
fn barrett_reduce(x: i64) -> i32 {
    ((x % Q as i64 + Q as i64) % Q as i64) as i32
}
...
impl From<[i32; N]> for Polynomial {
    ...
    fn from(coeffs: [i32; N]) -> Self {
        ...
        for coeff in &mut result {
            *coeff = barrett_reduce(*coeff as i64);
        }
        Self { coeffs: result }
    }
}
...
impl Mul<i32> for Polynomial {
    ...
    fn mul(self, scalar: i32) -> Self {
        let mut coeffs = [0i32; N];
        for i in 0..N {
            coeffs[i] = ((self.coeffs[i] as i64 * scalar as i64) % Q as i64) as i32;
        }
        Self { coeffs }
    }
}
...
impl Neg for Polynomial {
    ...
    fn neg(self) -> Self {
        let mut coeffs = [0i32; N];
        for i in 0..N {
            coeffs[i] = (-self.coeffs[i]).rem_euclid(Q);
        }
        Self { coeffs }
    }
}
