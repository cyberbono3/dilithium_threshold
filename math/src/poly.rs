...
/// let p: Polynomial<'_, FieldElement>  = poly![fe!(7); 5];
/// assert_eq!(5, p.degree() + 1);
/// for i in 0..5 {
///     assert_eq!(fe!(7), p.coefficients()[i]);
/// }
/// ```
///
/// From vectors:
    ...
    FF: FiniteField,
{
    ...
    /// use math::prelude::*;
    /// use num_traits::Zero;
    /// let f = Polynomial::new(fe_vec![1, 2, 3]);
    /// assert_eq!(Some(fe!(3)), f.leading_coefficient());
    /// ```
        ...
        <FF as Mul<FF2>>::Output: FiniteField,
    {
        ...
        let mut product =
            vec![<FF as Mul<FF2>>::Output::ZERO; degree_lhs + degree_rhs + 1];
    ...
    /// # use math::prelude::*;
    /// let f = Polynomial::new(fe_vec![1, 2, 3]);
    /// let g = f.scalar_mul(fe!(2));
    /// assert_eq!(Polynomial::new(fe_vec![2, 4, 6]), g);
    /// ```
    #[must_use]
    ...
    pub fn naive_divide(
        ...
        let Ok(quotient_degree) =
            usize::try_from(self.degree() - divisor.degree())
        else {
        ...
        for _ in 0..=quotient_degree {
            ...
            let remainder_degree =
                remainder_coefficients.len().saturating_sub(1);

            // skip divisor's leading coefficient: it has already been dealt with
            for (i, &divisor_coeff) in
                normal_rev_divisor.clone().skip(1).enumerate()
            {
                remainder_coefficients[remainder_degree - i] -=
                    quotient_coeff * divisor_coeff;
            }
        }
    ...
    pub fn norm_infinity(&self) -> u32 {
        self.coefficients()
            .iter()
            .map(|coeff| coeff.centered_absolute_value())
            .max()
            .unwrap_or(0)
    }
}
    ...
    FF: FiniteField + MulAssign<FieldElement>,
{
    ...
    const FAST_MODULAR_COSET_INTERPOLATE_CUTOFF_THRESHOLD_PREFER_LAGRANGE:
        usize = 1 << 8;
    ...
    pub fn fast_square(&self) -> Polynomial<'static, FF> {
        ...
        if degree == 0 {
            return Polynomial::from_constant(
    ...
    where
        FF: Mul<FF2>,
        FF2: FiniteField + MulAssign<FieldElement>,
        <FF as Mul<FF2>>::Output: FiniteField + MulAssign<FieldElement>,
    {
    ...
    pub fn reduce(
        ...
        if modulus.degree() < 0 {
            ...
            > FAST_REDUCE_MAKES_SENSE_MULTIPLE * modulus.degree()
        {
    ...
    pub fn fast_reduce(&self, modulus: &Self) -> Polynomial<'static, FF> {
        ...
        let (shift_factor_ntt, tail_size) =
            modulus.shift_factor_ntt_with_tail_length();
        ...
        if intermediate_remainder.degree() > 4 * modulus.degree() {
            ...
            intermediate_remainder = intermediate_remainder
                .reduce_by_structured_modulus(&structured_multiple);
        }
        ...
        FF: 'static,
    {
        ...
        let m = 1 + ntt_friendly_multiple
            ...
            .unwrap_or(0);
        let mut shift_factor_ntt =
            ntt_friendly_multiple.coefficients[..n].to_vec();
    ...
    pub fn reduce_by_ntt_friendly_modulus(
        ...
        let num_reducible_chunks = (self.coefficients.len()
            - (tail_length + chunk_size))
            .div_ceil(chunk_size);
        ...
        for chunk_index in (0..num_reducible_chunks).rev() {
            ...
            .concat();
            for (i, wwi) in
                working_window.iter_mut().enumerate().take(chunk_size)
            {
    ...
    fn structured_multiple(&self) -> Polynomial<'static, FF> {
        let n = usize::try_from(self.degree())
            .expect("cannot compute multiple of zero");
    ...
    pub fn structured_multiple_of_degree(
        &self,
        n: usize,
    ) -> Polynomial<'static, FF> {
        ...
        if degree == 0 {
            return Polynomial::new(
                [vec![FF::ZERO; n], vec![self.coefficients[0].inverse()]]
                    .concat(),
            );
        }
        ...
        // and arg = n - deg(f).
        let inverse_reverse =
            reverse.formal_power_series_inverse_minimal(n - degree);
    ...
    fn reduce_by_structured_modulus(
        &self,
        multiple: &Self,
    ) -> Polynomial<'static, FF> {
        assert_ne!(0, multiple.degree());
        let multiple_degree =
            usize::try_from(multiple.degree()).expect("cannot reduce by zero");
        assert_eq!(
            Some(FF::ONE),
        ...
        let window_stop =
            (tail_length + chunk_size) + num_reducible_chunks * chunk_size;
        ...
        for _ in (0..num_reducible_chunks).rev() {
            let overflow =
                Polynomial::new(working_window[tail_length..].to_vec());
            let product = overflow.multiply(&shift_polynomial);

            window_start -= chunk_size;
            working_window = [
                self.coefficients[window_start..window_start + chunk_size]
                    .to_vec(),
                working_window[0..tail_length].to_vec(),
            ]
            .concat();
    ...
    /// use math::prelude::*;
    /// let precision = 8;
    /// let f = Polynomial::new(fe_vec![42; precision]);
    ...
    pub fn formal_power_series_inverse_newton(
        self,
        precision: usize,
    ) -> Polynomial<'static, FF> {
        ...
        let switch_point =
            if Self::FORMAL_POWER_SERIES_INVERSE_CUTOFF < self_degree {
                0
            } else {
        ...
        if switch_point >= num_rounds {
            //return f;
            let mut coeffs = f.coefficients.into_owned();
        ...
        // final NTT domain
        let full_domain_length = ((1 << (num_rounds + 1))
            * self_degree as usize)
            .next_power_of_two();
        ...
        let mut current_domain_length =
            f.coefficients.len().next_power_of_two();

        // migrate to a larger domain as necessary
        let lde = |v: &mut [FF],
                   old_domain_length: usize,
                   new_domain_length: usize| {
        ...
        for _ in switch_point..num_rounds {
            ...
            if f_degree as usize >= current_domain_length {
                let next_domain_length =
                    (1 + f_degree as usize).next_power_of_two();
        ...
        // fix f_ntt.truncate(precision);
        // Truncate to requested precision (mod X^{precision}) before constructing the polynomial.
        f_ntt.truncate(precision);
    ...
    /// # Performance
    ///
    /// If possible, use a [base field element](FieldElement) as the offset.
    ///
    /// # Panics
    ///
    ...
    FF: FiniteField + MulAssign<FieldElement>,
{
    ...
    /// # use num_traits::Zero;
    /// # use math::prelude::*;
    /// let roots = fe_array![2, 4, 6];
    /// let zerofier = Polynomial::zerofier(&roots);
    ///
    /// assert_eq!(3, zerofier.degree());
    /// assert_eq!(fe_vec![0, 0, 0], zerofier.batch_evaluate(&roots));
    ///
    /// let  non_roots = fe_vec![0, 1, 3, 5];
    ...
    /// ```
    /// # use math::prelude::*;
    /// let domain = fe_vec![0, 1, 2, 3];
    /// let values = fe_vec![1, 3, 5, 7];
    /// let polynomial = Polynomial::interpolate(&domain, &values);
    ///
    /// assert_eq!(1, polynomial.degree());
    /// assert_eq!(fe!(9), polynomial.evaluate(fe!(4)));
    /// ```
    ///
    /// # Panics
    ///
    ...
    pub fn fast_interpolate(domain: &[FF], values: &[FF]) -> Self {
        ...
        let hadamard_mul = |x: &[_], y: Vec<_>| {
            x.iter().zip(y).map(|(&n, d)| n * d).collect_vec()
        };
    ...
    pub fn batch_fast_interpolate(
        ...
        let mut zerofier_dictionary: HashMap<(FF, FF), Polynomial<FF>> =
            HashMap::default();
        let mut offset_inverse_dictionary: HashMap<(FF, FF), Vec<FF>> =
            HashMap::default();
        ...
        let left_offset_inverse = match offset_inverse_dictionary.get(&left_key)
        {
            ...
            None => {
                let left_offset: Vec<FF> =
                    Self::batch_evaluate(&right_zerofier, &domain[..half]);
                let left_offset_inverse = FF::batch_inversion(left_offset);
                offset_inverse_dictionary
                    .insert(left_key, left_offset_inverse.clone());
                left_offset_inverse
            }
        };
        let right_offset_inverse = match offset_inverse_dictionary
            .get(&right_key)
        {
            Some(vector) => vector.to_owned(),
            None => {
                let right_offset: Vec<FF> =
                    Self::batch_evaluate(&left_zerofier, &domain[half..]);
                let right_offset_inverse = FF::batch_inversion(right_offset);
                offset_inverse_dictionary
                    .insert(right_key, right_offset_inverse.clone());
    ...
    pub fn batch_evaluate(&self, domain: &[FF]) -> Vec<FF> {
        if self.is_zero() {
            ...
            >= Self::REDUCE_BEFORE_EVALUATE_THRESHOLD_RATIO
                * (domain.len() as isize)
        {
    ...
    /// Only `pub` to allow benchmarking; not considered part of the public API.
    #[doc(hidden)]
    pub fn divide_and_conquer_batch_evaluate(
        &self,
    ...
    /// ```
    /// # use math::prelude::*;
    /// let f = Polynomial::new(fe_vec![0, 1, 2, 3, 4]); // 4x⁴ + 3x³ + 2x² + 1x¹ + 0
    /// let g = f.truncate(2);                            // 4x² + 3x¹ + 2
    /// assert_eq!(Polynomial::new(fe_vec![2, 3, 4]), g);
    /// ```
    ...
    /// assert_eq!(Polynomial::new(fe_vec![0, 1]), g);
    /// ```
    ...
    pub fn fast_modular_coset_interpolate_preprocess(
        ...
        let even_zerofiers = (0..n.ilog2())
            .map(|i| offset.inverse().mod_pow(1u32 << i))
            .zip(modular_squares.iter())
            .map(|(lc, sq)| {
                sq.scalar_mul(FF::from(lc.value() as u64)) - Polynomial::one()
            })
            .collect_vec();
        let odd_zerofiers = (0..n.ilog2())
            .map(|i| (offset * omega).inverse().mod_pow(1u32 << i))
            .zip(modular_squares.iter())
            .map(|(lc, sq)| {
                sq.scalar_mul(FF::from(lc.value() as u64)) - Polynomial::one()
            })
    ...
    pub fn fast_modular_coset_interpolate_with_zerofiers_and_ntt_friendly_multiple(
        ...
        if n < Self::FAST_MODULAR_COSET_INTERPOLATE_CUTOFF_THRESHOLD_PREFER_LAGRANGE {
            ...
            return interpolant
                .scale(<u32 as Into<FF>>::into(offset.inverse().value()))
        ...
        const MINUS_TWO_INVERSE: FieldElement = FieldElement::MINUS_TWO_INVERSE;
        let even_zerofier_on_odd_domain_inverted =
            vec![<u32 as Into<FF>>::into(MINUS_TWO_INVERSE.value()); n / 2];
        let odd_zerofier_on_even_domain_inverted =
            vec![<u32 as Into<FF>>::into(MINUS_TWO_INVERSE.value()); n / 2];
        ...
        let even_interpolant = Self::fast_modular_coset_interpolate(
            &even_domain_targets,
            offset,
            modulus,
        );
        let odd_interpolant = Self::fast_modular_coset_interpolate(
            &odd_domain_targets,
            offset * omega,
            modulus,
        );
        ...
        let interpolant = even_interpolant
            ...
            + odd_interpolant.multiply(
                &preprocessed.even_zerofiers[(n / 2).ilog2() as usize],
            );
...
where
    Ind: FiniteField
        + Mul<FieldElement, Output = Ind>
        + Sub<FieldElement, Output = Ind>,
    Coeff: FiniteField + Mul<Ind, Output = Eval>,
    Eval: FiniteField + Mul<Ind>,
{
    let root_order = codeword.len().try_into().unwrap();
    let generator = FieldElement::primitive_root_of_unity(root_order).unwrap();
    let domain_iter = (0..root_order).scan(FieldElement::ONE, |acc, _| {
    ...
    let domain_over_domain_shift = domain_iter
        ...
        .map(|(d, inv)| inv * d);
    let denominator =
        domain_over_domain_shift.clone().fold(Ind::ZERO, Ind::add);
...
impl<FF, FF2> Mul<Polynomial<'_, FF>> for FieldElement
where
    FF: FiniteField + Mul<FieldElement, Output = FF2>,
...
mod test_polynomials {
    ...
    type FePoly = Polynomial<'static, FieldElement>;

    impl proptest::arbitrary::Arbitrary for FePoly {
        type Parameters = ();
    ...
    fn polynomial_can_be_debug_printed() {
        let polynomial = Polynomial::new(fe_vec![1, 2, 3]);
        println!("{polynomial:?}");
    }
    ...
    fn polynomial_display_test() {
        fn polynomial<const N: usize>(coeffs: [u32; N]) -> FePoly {
            Polynomial::new(coeffs.map(FieldElement::new).to_vec())
        }
    ...
    #[proptest]
    fn leading_coefficient_of_zero_polynomial_is_none(
        #[strategy(0usize..30)] num_zeros: usize,
    ) {
        let coefficients = vec![FieldElement::ZERO; num_zeros];
    ...
    fn leading_coefficient_of_non_zero_polynomial_is_some(
        polynomial: FePoly,
        leading_coefficient: FieldElement,
        ...
        coefficients.extend(vec![FieldElement::ZERO; num_leading_zeros]);
    ...
    fn normalizing_canonical_zero_polynomial_has_no_effect() {
        let mut zero_polynomial = Polynomial::<FieldElement>::zero();
    ...
    fn normalizing_removes_spurious_leading_zeros(
        polynomial: FePoly,
        #[filter(!#leading_coefficient.is_zero())]
        leading_coefficient: FieldElement,
        #[strategy(0usize..30)] num_leading_zeros: usize,
    ) {
        let mut coefficients = polynomial.clone().coefficients.into_owned();
        ...
        let expected_num_coefficients =
            polynomial.coefficients.len() + num_inserted_coefficients;
    ...
    fn accessing_the_coefficients_is_equivalent_to_normalizing_then_raw_access(
        mut coefficients: Vec<FieldElement>,
        ...
        let accessed_coefficients_owned =
            polynomial.clone().into_coefficients();
    ...
    fn x_to_the_n_to_the_m_is_homomorphic(
        ...
        let to_the_n_times_m = Polynomial::<FieldElement>::x_to_the(n * m);
    ...
    fn shifting_polynomial_one_is_equivalent_to_raising_polynomial_x_to_the_power_of_the_shift(
        ...
        let x_to_the_shift =
            Polynomial::<FieldElement>::from([0, 1]).pow(shift as u32);
    ...
    fn polynomial_one_to_any_power_is_one(#[strategy(0u32..30)] exponent: u32) {
        let one_to_the_exponent =
            Polynomial::<FieldElement>::one().pow(exponent);
    ...
    fn pow_test() {
        ...
        let pol_squared =
            polynomial([0, 0, 196, 0, 112, 0, 240, 0, 148, 0, 88, 0, 48, 0, 9]);
        let pol_cubed = polynomial([
            0, 0, 0, 2744, 0, 2352, 0, 5376, 0, 4516, 0, 4080, 0, 2928, 0,
            1466, 0, 684, 0, 216, 0, 27,
        ]);
    ...
    #[proptest]
    fn polynomial_zero_is_neutral_element_for_addition(a: FePoly) {
        prop_assert_eq!(a.clone() + Polynomial::zero(), a.clone());
        prop_assert_eq!(Polynomial::zero() + a.clone(), a);
    }

    #[proptest]
    fn polynomial_one_is_neutral_element_for_multiplication(a: FePoly) {
        prop_assert_eq!(
            a.clone() * Polynomial::<FieldElement>::one(),
            a.clone()
        );
        prop_assert_eq!(Polynomial::<FieldElement>::one() * a.clone(), a);
    }

    #[proptest]
    fn multiplication_by_zero_is_zero(a: FePoly) {
    ...
    #[proptest]
    fn polynomial_addition_is_commutative(a: FePoly, b: FePoly) {
        prop_assert_eq!(a.clone() + b.clone(), b + a);
    }

    #[proptest]
    fn polynomial_multiplication_is_commutative(a: FePoly, b: FePoly) {
        prop_assert_eq!(a.clone() * b.clone(), b * a);
    }

    #[proptest]
    fn polynomial_addition_is_associative(a: FePoly, b: FePoly, c: FePoly) {
    ...
    #[proptest]
    fn polynomial_subtraction_of_self_is_zero(a: FePoly) {
        prop_assert_eq!(Polynomial::zero(), a.clone() - a);
    }

    #[proptest]
    fn polynomial_division_by_self_is_one(#[filter(!#a.is_zero())] a: FePoly) {
    ...
    fn leading_zeros_dont_affect_polynomial_division() {
        ...
        // x^3 - x + 1 / y = x
        let numerator = polynomial([1, FieldElement::P - 1, 0, 1]);
        let numerator_with_leading_zero =
            polynomial([1, FieldElement::P - 1, 0, 1, 0]);

        let divisor_normalized = polynomial([0, 1]);
        let divisor_not_normalized = polynomial([0, 1, 0]);
        let divisor_more_leading_zeros =
            polynomial([0, 1, 0, 0, 0, 0, 0, 0, 0]);

        let expected = polynomial([FieldElement::P - 1, 0, 1]);

        // Verify that the divisor need not be normalized
        assert_eq!(expected, numerator.clone() / divisor_normalized.clone());
        assert_eq!(
            expected,
            numerator.clone() / divisor_not_normalized.clone()
        );
    ...
    fn leading_coefficient_of_truncated_polynomial_is_same_as_original_leading_coefficient(
        ...
        let Some(lc) = poly.leading_coefficient() else {
            let reason =
                "test is only sensible if polynomial has a leading coefficient";
    ...
    fn truncated_polynomial_is_of_degree_min_of_truncation_point_and_poly_degree(
        ...
        let expected_degree =
            poly.degree().min(truncation_point.try_into().unwrap());
        prop_assert_eq!(
            expected_degree,
            poly.truncate(truncation_point).degree()
        );
    }
    ...
    fn truncating_zero_polynomial_gives_zero_polynomial(
        ...
        let poly =
            Polynomial::<FieldElement>::zero().truncate(truncation_point);
    ...
    fn truncation_negates_degree_shifting(
        ...
        #[strategy(..50_usize)] truncation_point: usize,
        #[filter(#poly.degree() >= #truncation_point as isize)] poly: FePoly,
    ) {
        prop_assert_eq!(
    ...
    fn polynomial_mod_some_power_of_x_shares_low_degree_terms_coefficients_with_original_polynomial(
        ...
        let remainder = poly.mod_x_to_the_n(power);
        let min_num_coefficients =
            poly.coefficients.len().min(remainder.coefficients.len());
        prop_assert_eq!(
    ...
    #[proptest]
    fn fast_multiplication_by_zero_gives_zero(poly: FePoly) {
        let product = poly.fast_multiply(&Polynomial::<FieldElement>::zero());
        prop_assert_eq!(Polynomial::zero(), product);
    }

    #[proptest]
    fn fast_multiplication_by_one_gives_self(poly: FePoly) {
        let product = poly.fast_multiply(&Polynomial::<FieldElement>::one());
        prop_assert_eq!(poly, product);
    }

    #[proptest]
    fn fast_multiplication_is_commutative(a: FePoly, b: FePoly) {
    ...
    #[proptest]
    fn batch_multiply_agrees_with_iterative_multiply(a: Vec<FePoly>) {
        let mut acc = Polynomial::one();
        for factor in &a {
    ...
    #[proptest]
    fn par_batch_multiply_agrees_with_batch_multiply(a: Vec<FePoly>) {
        prop_assert_eq!(
    ...
    fn naive_zerofier_and_fast_zerofier_are_identical(
        #[any(size_range(..Polynomial::<FieldElement>::FAST_ZEROFIER_CUTOFF_THRESHOLD * 2).lift())]
    ...
    fn zerofier_is_zero_only_on_domain(
        #[any(size_range(..1024).lift())] domain: Vec<FieldElement>,
        ...
        for point in domain {
            prop_assert_eq!(FieldElement::ZERO, zerofier.evaluate(point));
        }
        for point in out_of_domain_points {
            prop_assert_ne!(FieldElement::ZERO, zerofier.evaluate(point));
        }
    }

    #[proptest]
    fn zerofier_has_leading_coefficient_one(domain: Vec<FieldElement>) {
        let zerofier = Polynomial::zerofier(&domain);
        prop_assert_eq!(
            FieldElement::ONE,
    ...
    #[proptest]
    fn par_zerofier_agrees_with_zerofier(domain: Vec<FieldElement>) {
        prop_assert_eq!(
    ...
    fn fast_evaluate_on_hardcoded_domain_and_polynomial() {
        let domain = fe_array![6, 12];
        let x_to_the_5_plus_x_to_the_3 =
            Polynomial::new(fe_vec![0, 0, 0, 1, 0, 1]);

        let manual_evaluations =
            domain.map(|x| x.mod_pow(5) + x.mod_pow(3)).to_vec();
        let fast_evaluations =
            x_to_the_5_plus_x_to_the_3.batch_evaluate(&domain);
    ...
    fn interpolation_through_no_points_is_impossible() {
        let _ = Polynomial::<FieldElement>::interpolate(&[], &[]);
    }

    #[test]
    #[should_panic(expected = "zero points")]
    fn lagrange_interpolation_through_no_points_is_impossible() {
        let _ = Polynomial::<FieldElement>::lagrange_interpolate(&[], &[]);
    }
    ...
    fn lagrange_and_fast_interpolation_are_identical(
        ...
        #[strategy(vec(arb(), #domain.len()))] values: Vec<FieldElement>,
    ) {
        let lagrange_interpolant =
            Polynomial::lagrange_interpolate(&domain, &values);
    ...
    fn par_fast_interpolate_and_fast_interpolation_are_identical(
        ...
        let par_fast_interpolant =
            Polynomial::par_fast_interpolate(&domain, &values);
        let fast_interpolant = Polynomial::fast_interpolate(&domain, &values);
    ...
    #[proptest]
    fn add_assign_is_equivalent_to_adding_and_assigning(a: FePoly, b: FePoly) {
        let mut c = a.clone();
        c += b.clone();
    ...
    #[proptest]
    fn slow_and_normal_squaring_are_equivalent(poly: FePoly) {
        prop_assert_eq!(poly.slow_square(), poly.square());
    }

    #[proptest]
    fn normal_and_fast_squaring_are_equivalent(poly: FePoly) {
        prop_assert_eq!(poly.square(), poly.fast_square());
    }

    #[test]
    fn constant_zero_eq_constant_zero() {
        let zero_polynomial1 = Polynomial::<FieldElement>::zero();
        let zero_polynomial2 = Polynomial::<FieldElement>::zero();
    ...
    fn zero_polynomial_is_zero() {
        assert!(Polynomial::<FieldElement>::zero().is_zero());
    }
    ...
    fn zero_polynomial_is_zero_independent_of_spurious_leading_zeros(
        ...
        prop_assert_eq!(
            Polynomial::zero(),
            Polynomial::<FieldElement>::from(coefficients)
        );
    }

    #[proptest]
    fn no_constant_polynomial_with_non_zero_coefficient_is_zero(
        #[filter(!#constant.is_zero())] constant: FieldElement,
    ) {
    ...
    fn constant_one_eq_constant_one() {
        let one_polynomial1 = Polynomial::<FieldElement>::one();
        let one_polynomial2 = Polynomial::<FieldElement>::one();
    ...
    fn one_polynomial_is_one() {
        assert!(Polynomial::<FieldElement>::one().is_one());
    }
    ...
    fn no_constant_polynomial_with_non_one_coefficient_is_one(
        #[filter(!#constant.is_one())] constant: FieldElement,
    ) {
        let constant_polynomial = Polynomial::from_constant(constant);
    ...
    fn formal_derivative_of_zero_is_zero() {
        assert!(Polynomial::<FieldElement>::zero()
            .formal_derivative()
            .is_zero());
    }

    #[proptest]
    fn formal_derivative_of_constant_polynomial_is_zero(
        constant: FieldElement,
    ) {
        let formal_derivative =
            Polynomial::from_constant(constant).formal_derivative();
    ...
    fn formal_derivative_of_product_adheres_to_the_leibniz_product_rule(
        ...
        let product_formal_derivative =
            (a.clone() * b.clone()).formal_derivative();
        let product_rule =
            a.formal_derivative() * b.clone() + a * b.formal_derivative();
        prop_assert_eq!(product_rule, product_formal_derivative);
    }

    #[test]
    fn zero_is_zero() {
        let f = Polynomial::new(vec![FieldElement::new(0)]);
        assert!(f.is_zero());
    }
    ...
    fn formal_power_series_inverse_newton(
        ...
        let g = f.clone().formal_power_series_inverse_newton(precision);
        let mut coefficients = fe_vec![0; precision + 1];
        coefficients[precision] = FieldElement::ONE;
    ...
    //         FieldElement::new(3618372803227210457),
    //         FieldElement::new(14620511201754172786),
    //         FieldElement::new(2577803283145951105),
    //         FieldElement::new(1723541458268087404),
    //         FieldElement::new(4119508755381840018),
    //         FieldElement::new(8592072587377832596),
    //         FieldElement::new(236223201225),
    //     ]);
    //     let precision = 8;

    //     let g = f.clone().formal_power_series_inverse_newton(precision);
    //     let mut coefficients = vec![FieldElement::ZERO; precision + 1];
    //     coefficients[precision] = FieldElement::ONE;
    //     let xn = Polynomial::new(coefficients);
    //     let (_quotient, remainder) = g.multiply(&f).divide(&xn);
    //     assert!(remainder.is_one());
    ...
    fn structured_multiple_of_modulus_with_trailing_zeros_is_multiple(
        #[filter(!#raw_modulus.is_zero())] raw_modulus: FePoly,
    ...
    fn structured_multiple_of_degree_generates_structure(
        ...
        #[strategy(vec(arb(), 3..usize::min(30, #n)))] mut coefficients: Vec<
            FieldElement,
        >,
    ) {
        *coefficients.last_mut().unwrap() = FieldElement::ONE;
        let polynomial = Polynomial::new(coefficients);
        let structured_multiple = polynomial.structured_multiple_of_degree(n);

        let xn = Polynomial::new(
            [vec![FieldElement::ZERO; n], vec![FieldElement::ONE; 1]].concat(),
        );
    ...
    fn reverse_polynomial_with_nonzero_constant_term_twice_gives_original_back(
        ...
        let fx_plus_1 =
            f.shift_coefficients(1) + Polynomial::from_constant(fe!(1));
    ...
    fn reduce_by_structured_modulus_and_reduce_long_division_agree(
        ...
        #[strategy(vec(arb(), #m))] b_coefficients: Vec<FieldElement>,
        #[strategy(1usize..100)] _deg_a: usize,
        #[strategy(vec(arb(), #_deg_a + 1))] _a_coefficients: Vec<FieldElement>,
        ...
        *full_modulus_coefficients.last_mut().unwrap() = FieldElement::from(1);
        ...
        let structured_remainder =
            a.reduce_by_structured_modulus(&full_modulus);
    ...
    //     let m = 1;
    //     let a_coefficients = fe_vec![0, 0, 75944580];
    ...
    fn reduce_fast_and_reduce_long_division_agree(
        numerator: FePoly,
        #[filter(!#modulus.is_zero())] modulus: FePoly,
    ) {
        prop_assert_eq!(
    ...
    fn reduce_and_fast_reduce_long_division_agree_on_fixed_input() {
        ...
        for i in 1..100 {
            ...
            let roots = (0..i).map(FieldElement::new).collect_vec();
    ...
    fn reduce_long_division_and_fast_reduce_agree_simple_fixed() {
        let roots = (0..10).map(FieldElement::new).collect_vec();
    ...
    fn batch_evaluate_methods_are_equivalent(
        #[strategy(vec(arb(), (1<<10)..(1<<11)))] coefficients: Vec<
            FieldElement,
        >,
        #[strategy(vec(arb(), (1<<5)..(1<<7)))] domain: Vec<FieldElement>,
    ) {
        let polynomial = Polynomial::new(coefficients);
        let evaluations_iterative =
            polynomial.iterative_batch_evaluate(&domain);
        let zerofier_tree = ZerofierTree::new_from_domain(&domain);
        let evaluations_fast =
            polynomial.divide_and_conquer_batch_evaluate(&zerofier_tree);
        let evaluations_reduce_then =
            polynomial.reduce_then_batch_evaluate(&domain);
    ...
    fn structured_multiple_of_monomial_term_is_actually_multiple_and_of_right_degree(
        ...
        let coefficients =
            [fe_vec![0; degree], vec![leading_coefficient]].concat();
        let polynomial = Polynomial::new(coefficients);
    ...
    fn monomial_term_divided_by_smaller_monomial_term_gives_clean_division(
        ...
        #[filter(!#high_lc.is_zero())] high_lc: FieldElement,
        #[strategy(83..#high_degree)] low_degree: usize,
        #[filter(!#low_lc.is_zero())] low_lc: FieldElement,
    ) {
        let numerator =
            Polynomial::new([fe_vec![0; high_degree], vec![high_lc]].concat());
        let denominator =
            Polynomial::new([fe_vec![0; low_degree], vec![low_lc]].concat());
        let (quotient, remainder) = numerator.divide(&denominator);
        prop_assert_eq!(
            quotient
                .coefficients
    ...
    fn fast_modular_coset_interpolate_agrees_with_interpolate_then_reduce_property(
        ...
        let domain = (0..n)
            ...
            .collect_vec();
        prop_assert_eq!(
            Polynomial::fast_modular_coset_interpolate(
    ...
    fn fast_modular_coset_interpolate_agrees_with_interpolate_then_reduce_concrete(
        ...
        let logn = 8;
        let n = 1u32 << logn;
        let modulus = Polynomial::new(fe_vec![2, 3, 1]);
        let values = (0..n).map(|i| FieldElement::new(i / 5)).collect_vec();
        let offset = FieldElement::new(7);

        let omega = FieldElement::primitive_root_of_unity(n).unwrap();
        let mut domain = fe_vec![0; n as usize];
        domain[0] = offset;
    ...
    fn fast_modular_coset_interpolate_thresholds_relate_properly() {
        let intt = Polynomial::<FieldElement>::FAST_MODULAR_COSET_INTERPOLATE_CUTOFF_THRESHOLD_PREFER_INTT;
        let lagrange = Polynomial::<FieldElement>::FAST_MODULAR_COSET_INTERPOLATE_CUTOFF_THRESHOLD_PREFER_LAGRANGE;
        assert!(intt > lagrange);
    }

    #[proptest]
    fn interpolate_and_par_interpolate_agree(
        #[filter(!#points.is_empty())] points: Vec<FieldElement>,
        #[strategy(vec(arb(), #points.len()))] domain: Vec<FieldElement>,
    ) {
        let expected_interpolant = Polynomial::interpolate(&domain, &points);
        let observed_interpolant =
            Polynomial::par_interpolate(&domain, &points);
    ...
    fn fast_modular_coset_interpolate_respects_constant_modulus() {
        ...
        let lhs = Polynomial::fast_modular_coset_interpolate(
            &values, offset, &modulus,
        );
        let rhs = Polynomial::interpolate(&domain, &values).reduce(&modulus);
        assert_eq!(lhs, rhs); // both must be zero
        assert!(lhs.is_zero());
    }
}
